...
/*
 * This method get an action, the list of actions that should be executed and it looks for the missing actions.
 * The actions added in this way are called emotional actions
 */
...
void ActionAddition::instanceMissingActions(list<string> list_missing_actions,
	...
	for(list<string>::iterator it = list_missing_actions.begin();
		...
		boost::shared_ptr<PointPositionParameter> temp_position(new PointPositionParameter);
		boost::shared_ptr<ParameterVelocity> temp_velocity(new ParameterVelocity);
		shared_ptr<SimpleActionExecution> temp_action = instanceMissingAction(*it,
				temp_position,
				temp_velocity);
		temp_action->setTypeAction(TypeActionEmotional);
...
shared_ptr<SimpleActionExecution> ActionAddition::instanceMissingAction(string missing_action_name,
	...
	if(missing_action_name=="move_shoulder"){
		...
		temp_action_execution->setParameterTranslation(parameter_translation);
		return temp_action_execution;
	}else if(missing_action_name=="move_torso"){
		boost::shared_ptr<MoveTorsoExecution> temp_action_execution(new MoveTorsoExecution);
		temp_action_execution->setParameterPointPosition(boost::static_pointer_cast<PointPositionParameter>(parameter_position));
		temp_action_execution->setParameterTranslation(parameter_translation);
		return temp_action_execution;
	}else if(missing_action_name=="oscillate_torso"){
		boost::shared_ptr<OscillateTorsoExecution> temp_action_execution(new OscillateTorsoExecution);
		temp_action_execution->setParameterDestination(boost::static_pointer_cast<PointPositionParameter>(parameter_position));
		temp_action_execution->setParameterTranslation(parameter_translation);
		return temp_action_execution;

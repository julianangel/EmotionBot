...
unsigned long time_message = 80;
unsigned long time_message_upper = 100;
...
theatre_bot::MotorMessageTriskar motor_msg;
/*
...
void p_triskar(const theatre_bot::Vector32& p_value);

void stopRobot();
void timerHandler();

ros::Publisher motorTriskar("motor_triskar", &motor_msg);
...
ros::Subscriber<theatre_bot::Vector32> pTriskar("p_triskar", p_triskar);
...
{ 
 if ( (millis()-time_message) > 80){
    //Position Info
       ...
       motor_msg.set_motor.x = robot.getSetVelocityM1();
       motor_msg.set_motor.y = robot.getSetVelocityM2();
       motor_msg.set_motor.z = robot.getSetVelocityM3();
       motor_msg.odometry_motor.x = robot.getVelocityM1();
       motor_msg.odometry_motor.y = robot.getVelocityM2();
       motor_msg.odometry_motor.z = robot.getVelocityM3();
       motorTriskar.publish(&motor_msg);
...
/*
 * This function sets the d for the pid tuning
 * It changes the p and i value, p is store in x and i in y. The z describes the motor
 */
void p_triskar(const theatre_bot::Vector32& p_value){
  if(p_value.z == 1 ){
     robot.setPIDTuningMotor1(p_value.x,p_value.y,0.0);
  }else if(p_value.z == 2){
     robot.setPIDTuningMotor2(p_value.x,p_value.y,0.0);
  }else if(p_value.z == 3){
     robot.setPIDTuningMotor3(p_value.x,p_value.y,0.0);
  }else if(p_value.z == 0){
    robot.setPIDTuning(p_value.x,p_value.y,0.0);
  }
}
/*
 * This function gets the velocity and set the points

...
WalkCompoundAction::WalkCompoundAction() {
	...
	//Simple action oscillate shoulder
	...
	shared_ptr<PointPositionParameter> move_torso_parameter(new PointPositionParameter);
	//Simple action oscillate torso
	move_torso_simple_action.reset(new MoveTorsoSimpleAction);
	move_torso_simple_action->setActionIsPrincipal(false);
	//Parameter of the amplitude
	move_shoulder_parameter->setPointPosition(0.4,0.0,0.0);
	move_torso_parameter->setPointPosition(-0.2,0.0,0.0);
	oscillate_shoulder_simple_action->setParameterPointPosition(move_shoulder_parameter);
	move_torso_simple_action->setParameterPointPosition(move_torso_parameter);
	//Info of the action
	...
	this->abstract_actions.push_back(move_torso_simple_action);
	//this->abstract_actions.push_back(move_torso_simple_action);
	...
	this->robot_parts_used.merge(robot_parts_used_temp);
	robot_parts_used_temp = move_torso_simple_action->getRobotPartsUsed();
	this->robot_parts_used.merge(robot_parts_used);
...
list<shared_ptr<SimpleActionExecution> > WalkCompoundAction::Analyze(){
	...
	ROS_INFO("HAS FINISHED THE ACTION WALK");
	return temp;
}
...
void WalkCompoundAction::setParameterTrnaslation(shared_ptr<ParameterTranslation> parameter_translation){
	...
	this->move_torso_simple_action->setParameterTranslation(parameter_translation);

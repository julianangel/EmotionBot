...
TriskarSmallBody::TriskarSmallBody() {
	...
	velocity_rotate = 1.5; //rad/sec;
	reduce_linear_velocity = 200.0; //mm
	reduce_angular_velocity = 0.2; //rad
	linear_velocity = 600.0; //mm/sec
	x_error = 50.0; //mm
	y_error = 50.0; //mm
	has_arrived = false;
...
void TriskarSmallBody::generateEmotionalVelocityMove(){
	if(move_x.size()>0){
		...
		triskar_planner.setLinearVelocity(desire_velocity);
	}else{
		//std::cout<<"Velocity move "<<linear_velocity<<std::endl;
		triskar_planner.setLinearVelocity(linear_velocity);
	}
}
